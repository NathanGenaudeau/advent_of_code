...
for line in lines:
	if ('turn on ' in line):
		...
		for i in range(num1X, num2X + 1):
			for j in range(num1Y, num2Y + 1):
				matrix[i * int(math.sqrt(len(matrix))) + j] = matrix[i * int(math.sqrt(len(matrix))) + j] - 1 if matrix[i * int(math.sqrt(len(matrix))) + j] > 0 else matrix[i * int(math.sqrt(len(matrix))) + j]
	elif ('toggle' in line):
		[num1X, num1Y, num2X, num2Y] = splitLine(line, 'toggle ')
		for i in range(num1X, num2X + 1):
			for j in range(num1Y, num2Y + 1):
				matrix[i * int(math.sqrt(len(matrix))) + j] += 2
